...
int main()
{
	...
	times_c = abreArquivo(times_c, "countingsort_c.txt");
    times_d = abreArquivo(times_d, "countingsort_d.txt");
    times_r = abreArquivo(times_r, "countingsort_r.txt");

	plt::named_plot("Crescente", times_c);
    plt::named_plot("Decrescente", times_d);
    plt::named_plot("Aleatório", times_r);
    plt::title("Counting Sort - Comparação de entradas");
    plt::legend();
